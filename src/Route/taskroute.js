	...
	try {
		...
		await req.body.tasklist.every(async (tasks) => {
			...
			await Task.updateOne(
				{
					workid_backend: task.workid_backend,
					_id: tasks.taskid_backend,
				},
				{
					...task,
				},
...
router.patch("/multitaskupdate", workcheck, async (req, res) => {
	try {
		...
		Task.updateMany(
			{workid: req.work._id, _id: {$in: updatedid}},
			{completed: true},
			(error, response) => {
				if (response) {
					console.log("response =>", response);
...
router.delete("/multitaskdelete", workcheck, async (req, res) => {
	try {
		...
		Task.deleteMany(
			{workid: req.work._id, _id: {$in: deletedid}},
			(error, response) => {
				console.log("response =>", response);
			},
		);
